...
import { MapPin, CheckCircle, AlertCircle, Search } from 'lucide-react';
 ...
 */
const FloodMapPage = () => {
          const [searchFilter, setSearchFilter] = useState("western");
        const [searchTerm, setSearchTerm] = useState("");
      // List of default maps
      const defaultMaps = [
    ...
    // Delete extra map section by id
    const handleDeleteMapSection = (id) => {
      setExtraMaps(prev => prev.filter(map => map.id !== id));
      // If the deleted map was selected, reset to default
      if (extraMaps.find(m => m.id === id)?.name === mapView) {
        setMapView('western');
      }
    };
  ...
  return (
    ...
    >
      ...
      <div className="lg:col-span-2 mb-6">
        ...
        {/* End of header row */}
        {/* Interactive Map */}
        <div style={{ backgroundColor: '#1e293b', borderRadius: '12px', padding: '24px', minHeight: '500px', position: 'relative', marginBottom: '32px' }}>
          {mapView === 'western' && (
            <div style={{ textAlign: 'center', paddingTop: '100px' }}>
              ...
              <h3 style={{ color: '#f8fafc', fontSize: '20px', fontWeight: '600', marginBottom: '8px' }}>
                Western Priority Map
              </h3>
              <p style={{ color: '#94a3b8', fontSize: '14px', lineHeight: '1.6' }}>
                Showing western priority flood zones and related data
              </p>
            </div>
          )}
          {mapView === 'zones' && (
            <div style={{ textAlign: 'center', paddingTop: '100px' }}>
              ...
              <p style={{ color: '#94a3b8', fontSize: '14px', lineHeight: '1.6' }}>
                Showing all flood zones and risk areas
              </p>
            </div>
          )}
          {mapView === 'shelters' && (
            <div style={{ textAlign: 'center', paddingTop: '100px' }}>
              ...
              <p style={{ color: '#94a3b8', fontSize: '14px', lineHeight: '1.6' }}>
                Showing all active shelters and their capacity
              </p>
            </div>
          )}
          {mapView === 'teams' && (
            <div style={{ textAlign: 'center', paddingTop: '100px' }}>
              ...
              <p style={{ color: '#94a3b8', fontSize: '14px', lineHeight: '1.6' }}>
                Showing all rescue teams and their locations
              </p>
            </div>
          )}
          {/* Render extra map sections dynamically */}
          {extraMaps.map(map => (
            mapView === map.name && (
              <div key={map.id} style={{ textAlign: 'center', paddingTop: '100px' }}>
                <MapPin className="w-16 h-16 mx-auto mb-4" style={{ color: '#3b82f6' }} />
                <h3 style={{ color: '#f8fafc', fontSize: '20px', fontWeight: '600', marginBottom: '8px' }}>
                  {map.name}
                </h3>
                <p style={{ color: '#94a3b8', fontSize: '14px', lineHeight: '1.6' }}>
                  {map.type ? `Type: ${map.type}` : 'This is an additional map section added to the flood map.'}
                </p>
              </div>
            )
          ))}
        ...
        {/* Removed Edit and Add Map Section button as requested */}
        {/* Removed rendering of extra map sections below the main map. Only show selected map in main map area. */}
      </div>
      {/* Row for Critical Areas and Shelter Capacity */}
      <div className="flex flex-col lg:flex-row gap-6">
        {/* Critical Area Card */}
        <div style={{ backgroundColor: '#1e293b', borderRadius: '12px', border: '1px solid #334155', padding: '20px', flex: 1 }}>
