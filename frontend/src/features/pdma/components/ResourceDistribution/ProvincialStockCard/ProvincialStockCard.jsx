...
import { History, Package, Stethoscope, Home, Droplets } from 'lucide-react';
...
const getGaugeColor = (usagePercent) => {
    ...
    if (usagePercent <= 85) return '#f97316'; // Orange
    return '#ef4444'; // Red
};

/**
 * ProvincialStockCard Component
 * NDMA-style stock card with half-donut chart for PDMA provincial resources
 */
const ProvincialStockCard = ({
    resourceType,
    data,
    onViewHistory,
}) => {
    ...
    const available = data?.quantity || data?.available || 0;
    const allocated = data?.allocated || 0;
    const unit = data?.unit || 'units';

    // Normalize resource type to match config
    const normalizedType = resourceType?.toLowerCase().replace(/\s+/g, '').replace('supplies', '').replace('kits', '') || 'food';
    const typeKey = normalizedType.includes('food') ? 'food'
        : normalizedType.includes('medical') ? 'medical'
            : normalizedType.includes('shelter') ? 'shelter'
                : normalizedType.includes('water') ? 'water'
                    : 'food';

    const config = RESOURCE_CONFIG[typeKey] || { label: data?.name || resourceType, color: '#22c55e' };
    const Icon = RESOURCE_ICONS[typeKey] || Package;
    ...
    return (
        <div className={`national-stock-card ${status.class}`}>
            ...
            <button
                ...
                onClick={() => onViewHistory && onViewHistory({ name: config.label, type: typeKey })}
            >
...
export default ProvincialStockCard;
