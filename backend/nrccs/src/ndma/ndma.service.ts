...
import {
  ...
  NdmaAllocationStatus,
} from '../common/entities/ndma-resource-allocation.entity';
...
export class NdmaService {
  constructor(
    ...
    @Inject(
      forwardRef(
        () => require('../reasoning/reasoning.service').ReasoningService,
      ),
    )
  ...
  async createAlertFromPrediction(
    ...
    console.log(
      `[NDMA] ML Alert created: ${savedAlert.id} - ${prediction.flood_risk} risk for ${province.name}`,
    );
    ...
    if (
      ...
      (prediction.flood_risk === 'High' || prediction.flood_risk === 'Medium')
    ) {
      try {
        console.log(
          `[NDMA] Attempting to generate AI suggestions for ${province.name}...`,
        );
        ...
        console.log(
          `[NDMA] ✅ Generated ${suggestions.length} AI suggestions for ${province.name}`,
        );
      } catch (error) {
      ...
      console.log(
        `[NDMA] Skipping AI suggestions - reasoningService: ${!!this.reasoningService}, risk: ${prediction.flood_risk}`,
      );
    }
  ...
  async getProvinceSummaries(user: User) {
    ...
    const summaries = await Promise.all(
      provinces.map(async (province) => {
        ...
        const [activeSOS, activeAlerts, totalShelters, criticalDistricts] =
          await Promise.all([
            this.sosRepository.count({
              where: {
  ...
  async getAllShelters(user: User, status?: string, provinceId?: number) {
    ...
    if (provinceId) {
      ...
      const districtIds = districts.map((d) => d.id);
      if (districtIds.length > 0) {
        qb.andWhere('shelter.district_id IN (:...districtIds)', {
          districtIds,
        });
      } else {
        return [];
  ...
  async getSosStats(user: User) {
    const [pending, assigned, inProgress, completed, total] = await Promise.all(
      [
        this.sosRepository.count({
          where: { status: SosStatus.PENDING, isDeleted: false },
        }),
        this.sosRepository.count({
          where: { status: SosStatus.ASSIGNED, isDeleted: false },
        }),
        this.sosRepository.count({
          where: { status: SosStatus.IN_PROGRESS, isDeleted: false },
        }),
        this.sosRepository.count({
          where: { status: SosStatus.COMPLETED, isDeleted: false },
        }),
    ...
    const byPriority = await this.sosRepository
      ...
      .addSelect('COUNT(*)', 'count')
      .where('sos.is_deleted = false')
      .andWhere('sos.status IN (:...statuses)', {
        statuses: ['Pending', 'Assigned', 'En-route', 'In Progress'],
      })
  ...
  async getRescueTeamStats(user: User) {
    const [available, deployed, onMission, total] = await Promise.all([
      this.rescueTeamRepository.count({
        where: { status: RescueTeamStatus.AVAILABLE, isDeleted: false },
      }),
      this.rescueTeamRepository.count({
        where: { status: RescueTeamStatus.DEPLOYED, isDeleted: false },
      }),
      this.rescueTeamRepository.count({
        where: { status: RescueTeamStatus.ON_MISSION, isDeleted: false },
      }),
  ...
  async createNationalResource(
    createDto: CreateNationalResourceDto,
    user: User,
  ): Promise<Resource> {
    const resourceData = {
      name: createDto.name,
    ...
    const nationalResource = this.resourceRepository.create(
      resourceData as any,
  ...
  async increaseNationalStock(
    resourceId: number,
    increaseDto: IncreaseNationalStockDto,
    user: User,
  ) {
  ...
  async allocateResourceToProvince(
    resourceId: number,
    allocateDto: AllocateResourceToProvinceDto,
    user: User,
  ) {
    ...
    if (!nationalResource) {
      console.error(
        '[NDMA] ALLOCATE: National resource not found:',
        resourceId,
      );
      throw new NotFoundException('National resource not found');
    }
    ...
    if (!province) {
      console.error(
        '[NDMA] ALLOCATE: Province not found:',
        allocateDto.provinceId,
      );
    ...
    if (allocateDto.quantity > available) {
      console.error(
        '[NDMA] ALLOCATE: Insufficient stock. Requested:',
        allocateDto.quantity,
        'Available:',
        available,
      );
    ...
    const usagePercentage =
      (nationalResource.allocated / nationalResource.quantity) * 100;
    ...
    console.log(
      '[NDMA] ALLOCATE: National resource updated. New allocated:',
      nationalResource.allocated,
    );
    ...
    if (provinceResource) {
      ...
      console.log(
        '[NDMA] ALLOCATE: Province resource updated. New quantity:',
        provinceResource.quantity,
      );
    } else {
      ...
      provinceResource = this.resourceRepository.create(
        newResourceData as any,
      ) as unknown as Resource;
      await this.resourceRepository.save(provinceResource);
      console.log(
        '[NDMA] ALLOCATE: Province resource created. Quantity:',
        provinceResource.quantity,
      );
    }
    ...
    console.log(
      '[NDMA] ALLOCATE: Allocation record created. Allocation ID:',
      allocation.id,
    );
  ...
  async allocateResourceByType(
    allocateDto: AllocateResourceToProvinceDto & { resourceType: string },
    user: User,
  ) {
    // Defensive: Ensure user is provided
    if (!user || !user.id || !user.name) {
      console.error(
        '[NDMA] ALLOCATE: User context missing or invalid (byType)',
      );
      throw new Error('User context is required for resource allocation');
    }
    ...
    return await this.allocateResourceToProvince(
      nationalResource.id,
      allocateDto,
      user,
    );
  ...
  async reviewResourceRequest(
    requestId: number,
    reviewDto: ReviewResourceRequestDto,
    user: User,
  ) {
  ...
  async getNationalResources(user: User) {
    ...
    if (missingTypes.length > 0) {
      console.log(
        `[NDMA Treasury] Initializing missing core resources: ${missingTypes.join(', ')}`,
      );
