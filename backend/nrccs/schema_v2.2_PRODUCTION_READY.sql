...
-- =====================================================
-- TABLE 3.1: USER_SESSIONS (NEW - SESSION MANAGEMENT)
-- =====================================================
CREATE TABLE user_sessions (
    ...
    -- Ensure one active session per user (optional, remove for multi-device)
    UNIQUE(user_id, token_hash)
);

-- Index for session cleanup and validation
CREATE INDEX idx_sessions_expires ON user_sessions(expires_at);
CREATE INDEX idx_sessions_user ON user_sessions(user_id) WHERE expires_at > NOW();
